...
export function Landing() {
  ...
  const FEATURES = [
    {
      icon: Scan,
      title: t("feature_1_title", "Crop Registration"),
      desc: t(
        "feature_1_desc",
        "Quick entry of crop, quantity, harvest date, and location to start an analysis.",
      ),
    },
    {
      icon: Timer,
      title: t("feature_2_title", "Shelf-Life Prediction"),
      desc: t(
        "feature_2_desc",
        "ML-driven remaining safe storage days with a Green/Yellow/Red risk level.",
      ),
    },
    {
      icon: LineChart,
      title: t("feature_3_title", "Market Intelligence"),
      desc: t(
        "feature_3_desc",
        "Today's mandi price plus a 15-day forecast from a trained XGBoost model.",
      ),
    },
    {
      icon: Warehouse,
      title: t("feature_4_title", "Cold Storage Discovery"),
      desc: t(
        "feature_4_desc",
        "Map and list of nearby storage with real-time availability and pricing.",
      ),
    },
    {
      icon: Sparkles,
      title: t("feature_5_title", "Decision Engine"),
      desc: t(
        "feature_5_desc",
        "Compares sell-now vs. store vs. process and recommends the highest-profit path.",
      ),
    },
    {
      icon: Calculator,
      title: t("feature_6_title", "Profit Simulator"),
      desc: t(
        "feature_6_desc",
        "Side-by-side profit comparison across all three options with clear cost breakdowns.",
      ),
    },
    {
      icon: Cloud,
      title: t("feature_7_title", "Weather Intelligence"),
      desc: t(
        "feature_7_desc",
        "Temperature, humidity and rain risk that could accelerate spoilage.",
      ),
    },
    {
      icon: Mic,
      title: t("feature_8_title", "Voice Assistant"),
      desc: t("feature_8_desc", "Hindi voice interaction for farmers less comfortable typing."),
    },
    {
      icon: Landmark,
      title: t("feature_9_title", "Government Schemes"),
      desc: t(
        "feature_9_desc",
        "Relevant subsidy and insurance scheme awareness matched to your crop.",
      ),
    },
  ];
    ...
    <>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 pb-16 pt-14 sm:px-6 md:grid-cols-2 md:items-center md:pb-24 md:pt-20">
          <div className="relative z-10">
            <span className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">
              ...
              {t("hero_badge", "Post-Harvest Decision Intelligence")}
            </span>
            <motion.h1
              {...fade}
            ...
            <motion.p
              ...
              dangerouslySetInnerHTML={{
                __html: t(
                  "hero_subtitle",
                  'Knowing today\'s price isn\'t enough. FasalSeva AI tells you whether to <b class="text-foreground">sell now</b>, <b class="text-foreground">wait</b>, or <b class="text-foreground">store</b> — based on how long your crop will last and where prices are heading.',
                ),
              }}
            />
            <motion.div
            ...
            >
              ...
              >
                <Link to="/login">
                  {t("hero_cta_start", "Start Analysis")} <ArrowRight className="ml-2 h-4 w-4" />
                </Link>
              </Button>
              <Button size="lg" variant="outline">
                <PlayCircle className="mr-2 h-4 w-4" /> {t("hero_cta_demo", "Watch demo")}
              </Button>
            </motion.div>
            <div className="mt-10 grid max-w-lg grid-cols-2 gap-4 sm:grid-cols-4">
              {[
                { v: 30, s: "%", label: t("hero_stat_loss", "Post-harvest loss*") },
                {
                  ...
                  s: "B",
                  p: "₹",
                  label: t("hero_stat_money", "Annual distress-sale loss*"),
                },
                ...
                <div key={i} className="rounded-xl border border-border/60 bg-card p-4 shadow-card">
                  <div className="text-2xl font-bold tracking-tight">
                    {s.v === 0 ? (
                      ...
                      <>
                        <StatCounter value={s.v} suffix={s.s} prefix={s.p} />
                      </>
                    )}
                  </div>
            ...
            <p className="mt-3 text-[10px] text-muted-foreground">
              {t(
                "hero_disclaimer",
                "*Industry-cited estimates for illustrative context, not proprietary claims.",
              )}
            </p>
          </div>
          <motion.div {...fade} className="relative z-10">
            ...
            >
              <img
                src={heroImg}
                alt="Indian farmer with tomato in a green field at golden hour"
                width={1600}
                height={1100}
            ...
            >
              <p className="text-xs uppercase tracking-wide text-muted-foreground">
                {t("hero_recommendation_title", "Recommendation")}
              </p>
              <p className="mt-1 text-lg font-semibold flex items-center gap-2">
                <Sparkles className="h-4 w-4 text-primary" />{" "}
                {t("hero_recommendation_store", "Store Tomato · 8 days")}
              </p>
              <p className="text-xs text-primary font-medium mt-1 bg-primary/10 inline-block px-2 py-0.5 rounded-full">
                {t("hero_recommendation_profit", "Expected profit ↑ ₹5,140")}
              </p>
            </motion.div>
          </motion.div>
        </div>
        ...
        <div className="grid gap-10 md:grid-cols-2 md:items-center">
          <motion.div {...fade}>
            <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">
              {t("about_title", "The problem is a decision, not a data-point.")}
            </h2>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <motion.div {...fade} className="max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-wider text-primary">
              {t("features_badge", "What's inside")}
            </p>
            <h2 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">
              {t("features_title", "Nine capabilities, one decision.")}
            </h2>
            <p className="mt-3 text-muted-foreground">
              {t(
                "features_subtitle",
                "Each one is engineered to slot into a single recommendation — not another dashboard for you to interpret.",
              )}
            </p>
          </motion.div>
      ...
      <section id="workflow" className="mx-auto max-w-7xl px-4 py-20 sm:px-6">
        <motion.div {...fade} className="max-w-2xl">
          ...
          <h2 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">
            {t("workflow_title", "From input to recommendation in seconds.")}
          </h2>
        </motion.div>
          ...
          {WORKFLOW.map((w, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                <span className="grid h-9 w-9 place-items-center rounded-lg gradient-primary text-primary-foreground text-sm font-bold">
                  {i + 1}
                </span>
          ...
          <div className="relative flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div className="max-w-xl">
              <h3 className="text-3xl font-bold tracking-tight sm:text-4xl">
                {t("cta_title", "Ready to stop guessing?")}
              </h3>
              <p className="mt-3 text-primary-foreground/80">
                {t(
                  "cta_subtitle",
                  "Run your first analysis in under a minute — no signup required for the demo.",
                )}
              </p>
            </div>
