  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (step === "form") {
        ...
        if (otp.length < 4) { toast.error(t("auth.login_error_otp", "Enter the OTP")); setLoading(false); return; }
    ...
    <div className="grid min-h-screen md:grid-cols-2">
      <div className="flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-sm relative">
          ...
          <h1 className="text-3xl font-bold tracking-tight">{t("auth.signup_title", "Create your account")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">{t("auth.signup_subtitle", "Free during beta. No credit card.")}</p>
          <form onSubmit={submit} className="mt-8 space-y-4">
            {step === "form" && (
              <>
                <div className="space-y-2"><Label>{t("auth.name_label", "Name")}</Label>
                  <Input value={name} onChange={(e) => setName(e.target.value)} placeholder={t("auth.name_placeholder", "Ramesh Patel")} />
                </div>
                <div className="space-y-2"><Label>{t("auth.phone_number_label", "Phone")}</Label>
                  <div className="flex">
                ...
                <div className="space-y-2"><Label>{t("auth.role_label", "I am a")}</Label>
                  <div className="grid grid-cols-2 gap-2">
                    {(["farmer","storage_owner"] as const).map((r) => (
                      <button type="button" key={r} onClick={() => setRole(r)}
                        ...
                        <span className="font-semibold capitalize">{r === "farmer" ? t("auth.role_farmer", "Farmer") : t("auth.role_storage", "Storage owner")}</span>
                        <p className="mt-0.5 text-xs text-muted-foreground">{r === "farmer" ? t("auth.role_farmer_desc", "I grow & sell crops") : t("auth.role_storage_desc", "I run a cold storage")}</p>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            {step === "otp" && (
              <div className="space-y-2"><Label>{t("auth.otp_label_enter", "Enter OTP")}</Label>
            ...
            <Button disabled={loading} className="w-full gradient-primary text-primary-foreground">
              {loading ? "…" : step === "form" ? t("auth.send_otp", "Send OTP") : t("auth.verify_continue", "Verify & continue")}
            </Button>
          </form>
          <p className="mt-6 text-center text-sm text-muted-foreground">
            {t("auth.already_have_account", "Already have an account?")} <Link to="/login" className="font-semibold text-primary hover:underline">{t("auth.log_in", "Log in")}</Link>
          </p>
        </div>
      </div>
