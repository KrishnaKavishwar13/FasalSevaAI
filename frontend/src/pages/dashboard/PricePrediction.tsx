...
export function PricePrediction() {
    ...
    async function fetchCurrentPrice() {
      ...
      try {
        ...
        if (res.data.found && res.data.current_price > 0) {
          ...
          setPriceSource(
            t("price_prediction.live_agmarknet", "📡 Live from Agmarknet — {{market}} ({{date}})", {
          ...
          setPriceSource(
            t("price_prediction.price_not_found", "⚠️ Price not found — please enter manually"),
          );
        }
      } catch {
        setPriceSource(
          t("price_prediction.could_not_fetch", "⚠️ Could not fetch price — enter manually"),
        );
      }
    }
    fetchCurrentPrice();
  ...
  const recommendation = useMemo(() => {
    ...
    if (result.trend === "Increasing")
      return `Prices are expected to increase after 15 days. Delayed selling may increase profits.`;
    if (result.trend === "Stable")
      return `The market is expected to stay stable. A cautious selling strategy is recommended.`;
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const confidence = Math.min(
        96,
        Math.max(76, Math.round(82 + Math.abs(gain) / 100 + (trendStr === "Increasing" ? 4 : 0))),
      );
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      >
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight">
            {t("price_prediction.title", "Market Price Prediction")}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("price_prediction.subtitle", "Predict mandi price after the next 15 days.")}
          </p>
        </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.3em] text-muted-foreground">
                  {t("price_prediction.data_driven", "Data-driven market outlook")}
                </p>
                <h2 className="mt-2 text-2xl font-semibold">
                  {t("price_prediction.modern_forecasting", "Modern price forecasting")}
                </h2>
                <p className="mt-2 max-w-xl text-sm text-muted-foreground">
                  {t(
                    "price_prediction.plan_sale",
                    "Plan your sale timing with a forward-looking mandi price forecast tailored to your crop and region.",
                  )}
                </p>
              </div>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <Select
                  value={form.crop}
                  onValueChange={(value) =>
                    setForm((prev) => ({ ...prev, crop: value as PricePredictionInput["crop"] }))
                  }
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue placeholder={t("analysis.select_crop", "Select crop")} />
                  </SelectTrigger>
                  <SelectContent>
                    {CROPS.map((crop) => (
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setForm((prev) => ({ ...prev, current_price: Number(event.target.value) }))
                  }
                />
                {priceSource && (
                  <p
                    ...
                    style={{
                      color: priceSource.includes("Live") ? "var(--color-emerald-600)" : "#E8A020",
                    }}
                  >
                    {priceSource}
                  </p>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue placeholder={t("analysis.select_state", "Select state")} />
                  </SelectTrigger>
                  <SelectContent>
                    {STATES.map((state) => (
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue placeholder={t("price_prediction.select_month", "Select month")} />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue placeholder={t("price_prediction.select_week", "Select week")} />
                  </SelectTrigger>
                  <SelectContent>
              ...
              >
                {loading ? (
                  <>
                    ...
                    {t("shelf_life.predicting", "Predicting...")}
                  </>
                ) : (
                  t("price_prediction.predict_btn", "Predict Future Price")
                )}
              </Button>
        ...
        <div className="space-y-6">
          <Card className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.3em] text-muted-foreground">
                  {t("price_prediction.forecast", "Forecast")}
                </p>
                <h3 className="mt-1 text-xl font-semibold">
                  {t("price_prediction.live_market_outlook", "Live market outlook")}
                </h3>
              </div>
              ...
              >
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-border/60 bg-gradient-to-br from-primary/10 to-amber-500/10 p-4">
                    <p className="text-sm text-muted-foreground">
                      {t("price_prediction.todays_price", "Today’s Price")}
                    </p>
                  ...
                  <div className="rounded-2xl border border-border/60 bg-gradient-to-br from-emerald-500/10 to-primary/10 p-4">
                    <p className="text-sm text-muted-foreground">
                      {t("price_prediction.predicted_price", "Predicted Price")}
                    </p>
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  <div className="rounded-2xl border border-border/60 bg-background/70 p-4">
                    <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                      {t("price_prediction.expected_increase", "Expected Increase")}
                    </p>
                    <p className="mt-1 text-lg font-semibold">₹{result.difference}</p>
                  </div>
                  <div className="rounded-2xl border border-border/60 bg-background/70 p-4">
                    <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                      {t("price_prediction.confidence", "Confidence")}
                    </p>
                    <p className="mt-1 text-lg font-semibold">{result.confidence}%</p>
                  </div>
                  <div className="rounded-2xl border border-border/60 bg-background/70 p-4">
                    <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                      {t("price_prediction.trend", "Trend")}
                    </p>
                ...
                <div className="rounded-2xl border border-border/60 bg-background/70 p-4">
                  <div className="mb-3 flex items-center gap-2 text-sm font-semibold">
                    ...
                    {t("price_prediction.recommendation", "Recommendation")}
                  </div>
                ...
                <div className="rounded-2xl border border-border/60 bg-background/70 p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <p className="text-sm font-semibold">
                      {t("price_prediction.price_trend_chart", "Price trend line chart")}
                    </p>
              ...
              <div className="flex flex-col items-center justify-center space-y-4 rounded-xl border border-dashed border-border/60 bg-muted/20 p-8 py-16 text-center text-sm text-muted-foreground">
                ...
                <div>
                  <p className="font-medium text-foreground">
                    {t("price_prediction.no_forecast", "No price forecast yet")}
                  </p>
                  <p className="mt-1 max-w-[250px] text-xs leading-relaxed">
                    {t(
                      "price_prediction.no_forecast_desc",
                      "Submit the form to view a detailed projection of the future mandi price curve.",
                    )}
                  </p>
                </div>
              </div>
            ...
            <Card className="p-6">
              <div className="flex flex-wrap gap-2">
                <Button type="button" variant="outline" size="sm" onClick={handleDownload}>
                  ...
                  {t("price_prediction.download_report", "Download report")}
                </Button>
                <Button type="button" variant="outline" size="sm" onClick={handleShare}>
                  <Share2 className="mr-2 h-4 w-4" />{" "}
                  {t("price_prediction.share_result", "Share result")}
                </Button>
              </div>
            </Card>
          )}
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-muted-foreground">
              {t("price_prediction.prediction_history", "Prediction history")}
            </p>
            <h3 className="mt-1 text-xl font-semibold">
              {t("price_prediction.recent_predictions", "Recent predictions")}
            </h3>
          </div>
        ...
        {history.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border/60 bg-muted/30 p-6 text-sm text-muted-foreground">
            {t(
              "price_prediction.no_history",
              "No predictions yet. Run your first market forecast to build a trend history.",
            )}
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="text-sm text-muted-foreground">
                    ₹{entry.price_after_15_days} • {entry.confidence}%
                  </span>
                </div>
              </div>
            ))}
