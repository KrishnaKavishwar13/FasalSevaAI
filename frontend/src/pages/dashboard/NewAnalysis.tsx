  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-3xl font-bold tracking-tight">{t("analysis.new_title", "New analysis")}</h1>
        <p className="mt-1 text-sm text-muted-foreground">{t("analysis.new_subtitle", "Choose what you want to know about your crop.")}</p>
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Card className="group relative h-full flex flex-col overflow-hidden border-border/60 bg-card/90 p-6 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-card-hover">
            ...
            <div className="relative flex flex-col flex-grow">
              ...
              <h2 className="mt-5 text-xl font-semibold">{t("analysis.shelf_life_title", "Shelf Life")}</h2>
              <p className="text-sm font-medium text-primary mt-1">{t("analysis.shelf_life_hindi_desc", "फसल कितने दिन सुरक्षित है?")}</p>
              <p className="mt-2 text-sm leading-6 text-muted-foreground mb-4">{t("analysis.shelf_life_desc", "Know exactly how many days your crop will stay fresh.")}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <Card className="group relative h-full flex flex-col overflow-hidden border-border/60 bg-card/90 p-6 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-card-hover">
            ...
            <div className="relative flex flex-col flex-grow">
              ...
              <h2 className="mt-5 text-xl font-semibold">{t("analysis.price_forecast_title", "Price Forecast")}</h2>
              <p className="text-sm font-medium text-blue-600 mt-1">{t("analysis.price_forecast_hindi_desc", "मंडी भाव कब सबसे अच्छा होगा?")}</p>
              <p className="mt-2 text-sm leading-6 text-muted-foreground mb-4">{t("analysis.price_forecast_desc", "See predicted mandi price for next 15 days.")}</p>
          ...
          <Card className="group relative h-full flex flex-col overflow-hidden border-border/60 bg-card/90 p-6 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-card-hover ring-2 ring-amber-500">
            ...
            <div className="relative flex flex-col flex-grow">
              <div className="flex items-center justify-between gap-3">
                ...
                <Badge className="bg-amber-500 hover:bg-amber-600 text-white border-0">{t("analysis.recommended", "RECOMMENDED")}</Badge>
              </div>
              <h2 className="mt-5 text-xl font-semibold">{t("analysis.complete_title", "Complete Analysis")}</h2>
              <p className="text-sm font-medium text-amber-600 mt-1">{t("analysis.complete_hindi_desc", "पूरी सलाह एक जगह")}</p>
              <p className="mt-2 text-sm leading-6 text-muted-foreground mb-4">{t("analysis.complete_desc", "Shelf life + best selling day + cold storage + profit + AI advice.")}</p>
              
              <div className="mt-auto pt-4 flex flex-wrap items-center justify-between gap-3">
                <Button asChild className="w-full bg-amber-500 hover:bg-amber-600 text-white">
                  <Link to="/dashboard/new-analysis/complete">
                    {t("analysis.start_complete_flow", "Start Complete Flow")} <ArrowRight className="ml-2 h-4 w-4" />
                  </Link>
                </Button>
                <div className="flex items-center gap-2 text-xs text-muted-foreground w-full justify-center mt-2">
