...
export function ProfitSimulator() {
  ...
  const opts = [
    {
      key: "sell_now",
      label: t("profit.sell_today", "Sell Today"),
      ...data.profit_options.sell_now,
    },
    { key: "store", label: t("profit.store_crop", "Store Crop"), ...data.profit_options.store },
    {
      key: "process",
      label: t("profit.sell_to_processing", "Sell to Processing"),
      ...data.profit_options.process,
    },
  ];
  ...
  const chartData = opts.map((o) => ({
    ...
    profit: o.net_profit,
    fill: o.key === best.key ? "var(--color-primary)" : "var(--color-muted-foreground)",
  }));
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        <Button asChild variant="ghost" size="sm" className="mb-2 -ml-3">
          <Link to={`/dashboard/result/${id}`}>
            <ChevronLeft className="mr-1 h-4 w-4" /> {t("profit.back_to_result", "Back to result")}
          </Link>
        </Button>
        <h1 className="text-3xl font-bold tracking-tight">
          {t("profit.title", "Profit simulator")}
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {t("profit.subtitle", "Side-by-side comparison for {{crop}} · {{quantity}} kg.", {
            crop: data.crop,
        ...
        {opts.map((o) => (
          <Card
            ...
            className={cn(
              ...
              o.key === best.key
                ? "border-primary shadow-card-hover md:scale-[1.02]"
                : "opacity-90",
            )}
          >
            {o.key === best.key && (
            ...
            {o.key === "store" && data.spoilage.storage_viable === false ? (
              <div className="text-xs font-semibold text-red-500 mt-4 mb-2">
                {t(
                  "profit.spoil_fast_warning",
                  "⚠️ Crop spoils too fast — storage not recommended",
                )}
              </div>
            ) : (
              <dl className="mt-4 space-y-2 text-sm">
                ...
                <div className="mt-3 border-t border-border/60 pt-3">
                  <p className="text-xs uppercase tracking-wide text-muted-foreground">
                    {t("profit.net_profit", "Net profit")}
                  </p>
                  <p
                    className={cn(
                      "mt-1 text-2xl font-bold",
                      o.key === best.key ? "text-primary" : "",
                    )}
                  >
      ...
      <Card className="p-6">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">
          {t("profit.net_profit_comparison", "Net profit comparison")}
        </h3>
          ...
          <ResponsiveContainer>
            <BarChart data={chartData}>
              ...
              <Tooltip
                ...
                formatter={(v: number) => [formatINR(v), t("profit.net_profit", "Net profit")]}
              />
...
function Row({ label, v }: { label: string; v: number }) {
  return (
    <div className="flex justify-between">
      ...
      <dd className={cn("font-medium", v < 0 && "text-destructive")}>
        {v < 0 ? "−" : ""}
