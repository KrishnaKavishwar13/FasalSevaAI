    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      >
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight">
            {t("shelf_life.title", "Shelf-Life Prediction")}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {t(
              "shelf_life.subtitle",
              "Predict the remaining safe storage duration of harvested crops.",
            )}
          </p>
        </div>
            ...
            <div className="relative flex flex-wrap items-center justify-between gap-4">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.3em] text-muted-foreground">
                  {t("shelf_life.smart_storage", "Smart storage planning")}
                </p>
                <h2 className="mt-2 text-2xl font-semibold">
                  {t("shelf_life.premium_analysis", "Premium shelf-life analysis")}
                </h2>
                <p className="mt-2 max-w-xl text-sm text-muted-foreground">
                  {t(
                    "shelf_life.analysis_desc",
                    "Use AI-guided predictions to decide when to sell, store, or process your harvest safely.",
                  )}
                </p>
              </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <Select
                  value={form.crop}
                  onValueChange={(value) =>
                    setForm((prev) => ({ ...prev, crop: value as ShelfLifeInput["crop"] }))
                  }
                >
              ...
              <div>
                ...
                <Input
                  ...
                  type="number"
                  min="0"
                  value={form.days_stored}
                  onChange={(event) =>
                    setForm((prev) => ({ ...prev, days_stored: Number(event.target.value) }))
                  }
                />
              </div>
            </div>
              ...
              <div>
                ...
                <div className="mt-2 space-y-3 rounded-lg border border-border/60 bg-background/60 p-3">
                  <Slider
                    ...
                    min={40}
                    step={1}
                    onValueChange={([value]) => setForm((prev) => ({ ...prev, humidity: value }))}
                  />
                  <Input
                    type="number"
                    min="40"
                    ...
                    value={form.humidity}
                    onChange={(event) =>
                      setForm((prev) => ({ ...prev, humidity: Number(event.target.value) }))
                    }
                  />
                </div>
                <p className="text-xs text-soil/40 mt-1">
                  {t(
                    "shelf_life.humidity_hint",
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                {loading ? (
                  ...
                  t("shelf_life.predict_btn", "Predict Shelf Life")
                )}
              </Button>
        ...
        <div className="space-y-6">
          <Card className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.3em] text-muted-foreground">
                  {t("shelf_life.result", "Result")}
                </p>
                <h3 className="mt-1 text-xl font-semibold">
                  {t("shelf_life.live_prediction", "Live prediction")}
                </h3>
              </div>
              {result && (
              ...
              >
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-muted-foreground">
                        {t("shelf_life.remaining_safe_days", "Remaining Safe Days")}
                      </p>
                      <p className="text-4xl font-semibold">
                        {result.days_remaining}{" "}
                        <span className="text-base font-medium">
                          {t("shelf_life.days", "Days")}
                        </span>
                      </p>
                    </div>
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-xl border border-border/50 bg-background/70 p-3">
                      <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                        {t("shelf_life.confidence", "Confidence")}
                      </p>
                      <p className="mt-1 text-lg font-semibold">{result.confidence}%</p>
                    </div>
                    <div className="rounded-xl border border-border/50 bg-background/70 p-3">
                      <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">
                        {t("shelf_life.risk_level", "Risk Level")}
                      </p>
                ...
                <div className="rounded-2xl border border-border/60 bg-background/70 p-4">
                  <div className="flex items-center gap-2 text-sm font-semibold">
                    ...
                    {t("shelf_life.recommendation", "Recommendation")}
                  </div>
                ...
                <div className="rounded-2xl border border-border/60 bg-background/70 p-4">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold">
                      {t("shelf_life.risk_indicator", "Risk indicator")}
                    </p>
                    <Badge className={riskBadgeClass(result.risk_level)}>{result.risk_level}</Badge>
                  </div>
                  ...
                  <p className="text-center text-sm text-muted-foreground">
                    {t("shelf_life.confidence_score", "Confidence score")} {result.confidence}%
                  </p>
                </div>
              </motion.div>
            ) : (
              <div className="flex flex-col items-center justify-center space-y-4 rounded-xl border border-dashed border-border/60 bg-muted/20 p-8 py-16 text-center text-sm text-muted-foreground">
                ...
                <div>
                  <p className="font-medium text-foreground">
                    {t("shelf_life.no_analysis", "No analysis yet")}
                  </p>
                  <p className="mt-1 max-w-[250px] text-xs leading-relaxed">
                    {t(
                      "shelf_life.no_analysis_desc",
                      "Submit the form to generate a premium AI shelf-life report tailored to your live crop conditions.",
                    )}
                  </p>
                </div>
              </div>
            ...
            <Card className="p-6">
              <div className="flex flex-wrap gap-2">
                <Button type="button" variant="outline" size="sm" onClick={handleDownload}>
                  ...
                  {t("shelf_life.download_report", "Download report")}
                </Button>
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-muted-foreground">
              {t("shelf_life.history", "Prediction history")}
            </p>
            <h3 className="mt-1 text-xl font-semibold">
              {t("shelf_life.recent_predictions", "Recent predictions")}
            </h3>
          </div>
        ...
        {history.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border/60 bg-muted/30 p-6 text-sm text-muted-foreground">
            {t(
              "shelf_life.no_history",
              "No predictions yet. Run your first analysis to build a storage history.",
            )}
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {entry.days_remaining} days • {entry.confidence}%
                  </span>
                </div>
              </div>
            ))}
