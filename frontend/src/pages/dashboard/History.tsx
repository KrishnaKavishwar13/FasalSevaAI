    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{t("history.subtitle", "All your past analyses.")}</p>
        </div>
        <div className="relative">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input placeholder={t("history.search", "Search crop…")} className="w-64 pl-9" value={q} onChange={e => setQ(e.target.value)} />
        </div>
      </div>
        ...
        {isLoading ? <div className="p-6 space-y-2">{[1,2,3].map(i => <Skeleton key={i} className="h-10" />)}</div>
          : filtered.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="mt-4 text-sm text-muted-foreground">{t("history.no_analyses", "No analyses yet.")}</p>
              <Button asChild className="mt-4 gradient-primary text-primary-foreground"><Link to="/dashboard/new-analysis">{t("history.run_first", "Run your first analysis")}</Link></Button>
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t("analysis.crop_label", "Crop")}</TableHead>
                  <TableHead>{t("history.date", "Date")}</TableHead>
                  <TableHead>{t("history.risk", "Risk")}</TableHead>
                  <TableHead>{t("history.recommendation", "Recommendation")}</TableHead>
                  <TableHead className="text-right">{t("history.profit", "Profit")}</TableHead>
                  <TableHead />
                </TableRow>
              </TableHeader>
              <TableBody>
                {filtered.map(h => (
                  <TableRow key={h.id}>
                    ...
                    <TableCell className="text-right">
                      <Button asChild variant="ghost" size="sm"><Link to={`/dashboard/result/${h.id}`}>{t("history.view", "View")} <ArrowRight className="ml-1 h-3 w-3" /></Link></Button>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
