      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {t("history.subtitle", "All your past analyses.")}
          </p>
        </div>
        <div className="relative">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input
            placeholder={t("history.search", "Search crop…")}
            className="w-64 pl-9"
            value={q}
              ...
              {filtered.map((h) => (
                <TableRow key={h.id}>
                  <TableCell className="font-medium">
                    {h.crop} <span className="text-muted-foreground">· {h.quantity_kg} kg</span>
                  </TableCell>
                  ...
                  <TableCell className="text-right">
                    <Button asChild variant="ghost" size="sm">
                      <Link to={`/dashboard/result/${h.id}`}>
                        {t("history.view", "View")} <ArrowRight className="ml-1 h-3 w-3" />
                      </Link>
                    </Button>
                  </TableCell>
                </TableRow>
