...
export function CompleteAnalysis() {
  ...
  useEffect(() => {
    const fetchGPSAndWeather = async () => {
      try {
        ...
        const w = await fetch(
          `https://api.open-meteo.com/v1/forecast?latitude=${cLat}&longitude=${cLng}&current=temperature_2m,relative_humidity_2m&forecast_days=1`,
        ).then((r) => r.json());
        setForm((f) => ({
          ...f,
        ...
        toast.error(
          t("analysis.gps_weather_failed", "Could not fetch GPS/Weather. Using default values."),
        );
      } finally {
  ...
  useEffect(() => {
    if (form.crop && form.state) {
      ...
      apiClient
        .get(`/current-price?crop=${form.crop}&state=${form.state}`)
        .then((r) => {
          if (r.data.found) {
            ...
            if (r.data.is_fallback) {
              setPriceNote(
                t(
                  "analysis.price_unavailable_seasonal",
                  "⚠️ Live price unavailable — using seasonal average. Please verify manually.",
                ),
              );
            } else {
              setPriceNote(
                t(
                  "analysis.live_from_agmarknet_market",
                  "✅ Live from Agmarknet — {{market}} ({{date}})",
                  { market: r.data.market, date: r.data.date },
                ),
              );
            }
          } else {
            ...
            toast.info(
              t("analysis.approx_market_price", "Using approximate market price for {{crop}}", {
                crop: form.crop,
              }),
            );
          }
        ...
        .catch(() => {
          ...
          toast.error(
            t(
              "analysis.price_fetch_failed",
              "Could not fetch live price. Using approximate price.",
            ),
          );
        })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.price || !form.temp || !form.humidity) {
      return toast.error(
        t(
          "analysis.fill_auto_fields",
          "Please fill in all auto-fetched fields if they failed to load.",
        ),
      );
    }
    ...
    try {
      ...
      console.error(err);
      toast.error(
        t("analysis.analysis_failed", "Analysis failed. ") + (err.response?.data || err.message),
      );
    } finally {
      setLoading(false);
    }
      ...
      >
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {t("analysis.complete_desc_engine", "End-to-end post harvest decision engine.")}
          </p>
        </div>
      </motion.div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
            <div>
              ...
              <Select
                value={form.crop}
                onValueChange={(value) => setForm((f) => ({ ...f, crop: value }))}
              >
          ...
          <div className="grid gap-6 md:grid-cols-3">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setForm((f) => ({ ...f, days_stored: e.target.value }))}
                required
              />
            </div>
            ...
            <div className="relative">
              ...
              <p
                className={`text-xs mt-1 ${priceNote.includes("⚠️") ? "text-amber-600" : "text-green-600"}`}
              >
                {priceNote}
              </p>
            </div>
            ...
            <div className="relative">
              ...
              <p className="text-xs text-amber-600 mt-1 flex items-center gap-1">
                <MapPin className="h-3 w-3" /> {t("analysis.auto_from_gps", "Auto from GPS")}
              </p>
            </div>
          </div>
          ...
          <Button
            type="submit"
            className="w-full h-12 bg-amber-500 hover:bg-amber-600 text-white text-lg"
            disabled={loading || weatherLoading || priceLoading}
          >
            {loading ? (
              <>
              ...
              t("analysis.run_complete_analysis", "Run Complete AI Analysis")
            )}
          </Button>
        </form>
      </Card>
