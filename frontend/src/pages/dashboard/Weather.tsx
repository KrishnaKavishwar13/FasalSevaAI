  ...
  useEffect(() => {
    const fetchWeather = async () => {
      ...
      try {
        ...
        const w = await fetch(
          `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=temperature_2m,relative_humidity_2m,precipitation&forecast_days=1`,
        ).then((r) => r.json());
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">
          {t("weather.title", "Weather & Storage Conditions")}
        </h1>
        <p className="mt-1 text-sm text-muted-foreground flex items-center gap-1">
          <Navigation className="h-4 w-4" /> {t("weather.subtitle", "Live from GPS coordinates")}
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <Card className="p-6 relative overflow-hidden bg-sky-50 dark:bg-sky-950/20 border-sky-200 dark:border-sky-900/30">
            ...
            <h2 className="text-xl font-bold flex items-center gap-2 text-sky-900 dark:text-sky-300">
              <CloudRain className="h-6 w-6" /> {t("weather.outdoor", "Outdoor Weather")}
            </h2>
            <p className="text-sm text-sky-700/80 dark:text-sky-400/80 mb-6">
              {t("weather.outdoor_desc", "Current conditions at your farm")}
            </p>
          ...
          <Card className="p-6 relative overflow-hidden bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-900/30">
            ...
            <h2 className="text-xl font-bold flex items-center gap-2 text-amber-900 dark:text-amber-400">
              <Home className="h-6 w-6" /> {t("weather.storage", "Storage Conditions")}
            </h2>
            <p className="text-sm text-amber-700/80 dark:text-amber-400/80 mb-6">
              {t("weather.storage_desc", "Estimated indoor ambient conditions")}
            </p>
          ...
          <Card className="p-6 mt-6 border-emerald-200 dark:border-emerald-900/30">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-xl font-bold">
                  {t("weather.how_affects", "How this affects your {{crop}}", {
                    crop: user.mainCrop,
                  })}
                </h3>
                <p className="text-sm text-muted-foreground">
                  {t(
                    "weather.affects_desc",
                    "Based on the current storage conditions calculated above.",
                  )}
                </p>
              </div>
            </div>
            ...
            {impact ? (
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-muted/40 flex items-center justify-between">
                  <span className="text-sm font-medium">
                    {t("weather.safe_shelf_life", "Expected Safe Shelf Life")}
                  </span>
                  <span className="text-xl font-bold">
                    {impact.days_remaining} {t("result.days", "days")}
                  </span>
                </div>
                <div className="p-4 rounded-2xl bg-muted/40 flex items-center justify-between">
                  <span className="text-sm font-medium">
                    {t("weather.spoilage_risk", "Current Spoilage Risk")}
                  </span>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground p-4 bg-muted/30 rounded-2xl">
                ...
                {t("weather.analysis_failed", "Spoilage model analysis failed to load.")}
              </div>
            )}
          </Card>
        </motion.div>
