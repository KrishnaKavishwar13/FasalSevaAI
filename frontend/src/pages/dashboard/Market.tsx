...
export function Market() {
  ...
  useEffect(() => {
    if (crop && state) {
      ...
      apiClient
        .get(`/current-price?crop=${crop}&state=${state}`)
        .then((r) => {
          if (r.data.found) {
            ...
            if (r.data.is_fallback) {
              setPriceNote(
                t(
                  "market.price_fallback",
                  "⚠️ Live price unavailable — using seasonal average. Please verify manually.",
                ),
              );
            } else {
              setPriceNote(
                t("market.live_price", "✅ Live from Agmarknet — {{market}} ({{date}})", {
  ...
  const { data, isFetching, refetch } = useQuery({
    ...
    queryFn: () =>
      marketService.getPriceForecast({
        crop,
        state,
        current_price: price,
        month: new Date().getMonth() + 1,
        week: Math.ceil((new Date().getDate() + 1) / 7),
      }),
  });
  ...
  const combined = data
    ? data.daily.map((d, i) => ({
        day: d.day,
        local: d.price,
        regional: data.regional_avg[i]?.price,
      }))
    : [];
  const trend = data
    ? data.price_after_15_days > price * 1.03
      ? "Increasing"
      : data.price_after_15_days < price * 0.97
        ? "Decreasing"
        : "Stable"
    : "Stable";
  const TrendIcon =
    trend === "Increasing" ? ArrowUpRight : trend === "Decreasing" ? ArrowDownRight : ArrowRight;
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {t("market.subtitle", "Live Agmarknet prices + 15-day AI mandi price forecast.")}
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="p-6 md:col-span-2">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => {
                  setState(v);
                  setDistrict(STATE_DISTRICTS[v][0]);
                }}
              >
                <SelectTrigger className="mt-1">
                  <SelectValue />
            ...
            <div>
              <Label>{t("market.district", "District")}</Label>
            ...
            <div className="relative">
              <Label>{t("market.todays_price", "Today's price (₹/qtl)")}</Label>
              <Input
                type="number"
                className="mt-1"
                value={price}
          ...
          <Button
            className="mt-6 w-full sm:w-auto gradient-primary text-primary-foreground"
            onClick={() => refetch()}
            disabled={isFetching || priceLoading}
          >
            {isFetching
              ? t("market.loading", "Loading…")
              : t("market.update_forecast", "Update forecast")}
          </Button>
        </Card>
        ...
        <Card className="p-6 bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-900 flex flex-col justify-center">
          <p className="text-sm font-semibold text-green-700 dark:text-green-400 flex items-center gap-2">
            <MapPin className="h-4 w-4" /> {t("market.mandi", "{{state}} Mandi", { state })}
          </p>
          <p className="text-4xl font-bold text-green-700 dark:text-green-300 mt-2">
            ₹{price}
            <span className="text-xl text-green-600/70">{t("market.per_qtl", "/qtl")}</span>
          </p>
          <p
            className={`text-xs mt-2 ${priceNote.includes("⚠️") ? "text-amber-600" : "text-green-600"}`}
          >
            {priceNote}
          </p>
          <div className="mt-4 flex gap-4 text-sm text-green-700 dark:text-green-400">
            <div>
              <span className="opacity-70">{t("market.min", "Min:")}</span> ₹{price - 150}
            </div>
            <div>
              <span className="opacity-70">{t("market.max", "Max:")}</span> ₹{price + 200}
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold">
            {t("market.chart_title", "15-Day Price Forecast")}
          </h3>
          <span
            className={cn(
