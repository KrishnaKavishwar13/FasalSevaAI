...
export function ColdStorage() {
  ...
  const { data: facilities, isLoading } = useQuery({
    ...
    queryFn: () => storageService.getFacilities({ crop: crop === "all" ? undefined : crop, maxDistance: 500 }),
    enabled: locationReady,
  });

  const nearbyStorages = useMemo(() => {
    if (!facilities) return [];
    return facilities.filter(f => f.distance_km <= maxDistance).sort((a,b) => a.distance_km - b.distance_km);
